...
let editIndex = -1;
let pieChart; // For Chart.js instance
...
function loadData() {
  ...
  updatePieChart(); // Update chart when data changes
}
...
function showBuyerSummary(buyerTotals, total) {
  ...
  buyers.forEach(buyerKey => {
    ...
    if (diff > 0) {
      html += `<li><strong>${buyer.originalName}</strong> paid ₹${paid.toFixed(2)}, which is ₹${diff.toFixed(2)} more than their share.</li>`;
    } else if (diff < 0) {
      html += `<li><strong>${buyer.originalName}</strong> paid ₹${paid.toFixed(2)}, which is ₹${(-diff).toFixed(2)} less than their share.</li>`;
    } else {
...
function updatePieChart() {
  ...
  data.forEach(entry => {
    if (categoryTotals[entry.item]) {
      categoryTotals[entry.item] += entry.price;
    } else {
      categoryTotals[entry.item] = entry.price;
    }
  });
  ...
  const colors = [
    ...
    '#FFA726', '#AB47BC', '#26C6DA', '#EF5350', '#8D6E63'
  ];

  const ctx = document.getElementById('categoryPieChart').getContext('2d');

  if (pieChart) {
    pieChart.destroy(); // destroy old chart
  }
